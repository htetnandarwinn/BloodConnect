...
<head>
    ...
    <style>
        body {
            ...
            background: radial-gradient(100% 100% at 50% 0%, #ffffff 50%, #f8fafc 100%);
        }
    ...
    <div class="flex-1 flex flex-col min-h-screen">
        ...
        <main class="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl w-full mx-auto space-y-6 sm:space-y-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                <div class="bg-white border border-slate-100 p-5 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm hover:shadow-xl hover:shadow-slate-200/50 hover:-translate-y-1 transition-all duration-300 flex items-center gap-5 group animate-slide-up [animation-delay:100ms] opacity-0 [animation-fill-mode:forwards]">
                ...
                <div class="bg-white border border-slate-100 p-5 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm hover:shadow-xl hover:shadow-slate-200/50 hover:-translate-y-1 transition-all duration-300 flex items-center gap-5 group animate-slide-up [animation-delay:150ms] opacity-0 [animation-fill-mode:forwards]">
                ...
                <div class="bg-white border border-slate-100 p-5 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm hover:shadow-xl hover:shadow-slate-200/50 hover:-translate-y-1 transition-all duration-300 flex items-center gap-5 group sm:col-span-2 lg:col-span-1 animate-slide-up [animation-delay:200ms] opacity-0 [animation-fill-mode:forwards]">
            ...
            <div class="space-y-4 animate-slide-up [animation-delay:250ms] opacity-0 [animation-fill-mode:forwards]">
                ...
                <div class="space-y-4">
                    <?php foreach ($blood_requests as $index => $request): ?>
                        ...
                        <div class="bg-white border <?= $priorityColor; ?> border-slate-100 p-4 sm:p-5 rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 flex flex-col lg:flex-row lg:items-center justify-between gap-4 animate-slide-up" style="animation-delay: <?= 300 + ($index * 50); ?>ms;">
                            ...
                            <div class="flex items-start gap-4 min-w-0">
                                ...
                                <div class="space-y-1 min-w-0">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <h4 class="font-bold text-slate-900 text-sm sm:text-base truncate">Request from: <?= htmlspecialchars($request['requester']); ?></h4>
                                        <span class="text-[9px] px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider <?= $badgeColor; ?>">
                                            <?= htmlspecialchars($request['priority']); ?> Priority
                                        </span>
                                    </div>
                                    
                                    <div class="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-4 text-xs sm:text-sm text-slate-500">
                                        <p class="truncate"><i class="fa-solid fa-hospital mr-1 text-slate-400 text-xs"></i> <?= htmlspecialchars($request['location']); ?></p>
                                        <span class="hidden sm:inline text-slate-300">•</span>
                                        <p class="font-medium shrink-0">
                                            <span class="font-bold text-slate-900"><?= htmlspecialchars($request['units']); ?></span> Units needed by <span class="text-red-600 font-semibold"><?= htmlspecialchars($request['deadline']); ?></span>
                                        </p>
                                    </div>
                                </div>
                            </div>
